
<div align="center">
  <h1>Atividade Prática  04/08</h1>
</div>

## Situação
Imagine que uma escola deseja criar um sistema de controle de alunos.

   1. Quem utilizará o sistema?
   2. Cite 3 requisitos funcionais (o que o sistema deve fazer)
   3. Cite 2 requisitos não funcionais (como o sistema deve funcionar).
   4. Explique, em poucas palavras, por que é importante levantar os requisitos antes de desenvolver um software.

## Resolução

> **Foi desenvolvido o levantamento de requisitos para entrada e saída de alunos matriculados.**

   1. Diretoria; Coordenação.
   2. Registrar alunos; Consultar registros de Entrada e Saída; Associar fluxo de Entrada e Saída á lista de presença de alunos.
   3. Compatibilidade Multiplataforma; Executar o sistema offline.
   4. É importante levantar os requisitos antes de desenvolver o software para evitar desenvolver um sistema que o cliente não pediu e haver perda de investimento financeiro. 
